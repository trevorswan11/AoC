...
std::vector<std::pair<int, int>> parse_rules(const std::vector<std::string>& lines)
{
...
bool is_valid_update(const std::vector<int>& update, const std::vector<std::pair<int, int>>& rules)
{
    ...
    {
        if (index_map.count(a) && index_map.count(b) && index_map[a] >= index_map[b])
        {
            return false;
        }
    }
...
void part_one(const std::vector<std::string>& rule_lines, const std::vector<std::string>& update_lines)
{
...
std::vector<int> topological_sort(const std::vector<int>& update, const std::vector<std::pair<int, int>>& all_rules) {
...
void part_two(const std::vector<std::string>& rule_lines, const std::vector<std::string>& update_lines)
{
    auto rules = parse_rules(rule_lines);
    int total_valid = 0;
